...
export default tseslint.config(
  {
    ignores: [
      ...
      '**/*.d.ts',
      '**/*.tsbuildinfo',
      // Generated by `pnpm run build:engine` (M1-02) — a bundled copy of
      // packages/flight-sim, not source. See scripts/build-engine.mjs.
  ...
  // ── The Edge Function runs on Deno, not Node ───────────────────────────────
  // supabase/functions/**/*.ts is neither built by `tsc -b` (it is not a
  // project reference — Deno has its own type-checker) nor executed by Node
  // outside tests. `handler.ts` is plain, runtime-agnostic TypeScript and is
  // covered by the generic ruleset above; this block only adds the Fetch API
  // globals (real in both Deno and Node 22) that a Deno entry point needs.
  {
