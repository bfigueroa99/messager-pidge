...
describe('LossScreen', () => {
  ...
  it('[M1-22] the loss screen names the place and time and never shows the text', async () => {
    jest.useFakeTimers();
    ...
    expect(
      screen.getByText('Wren did not arrive. Taken near Altoona, Pennsylvania, at 11:41 PM. The note was not recovered.'),
    ).toBeTruthy();
    expect(screen.queryByText('See you soon.')).toBeNull();
  });

  it('[M1-22] a cold start after loss shows the memorial with no animation', async () => {
    ...
    expect(screen.getByText(/Altoona, Pennsylvania/)).toBeTruthy();
  });

  it('[M1-22] never reveals the memorial for a flight that resolved as delivered', async () => {
    jest.useFakeTimers();
    let nowMs = FLIGHT.arrivesAtMs - 500;
  ...
  it('[M1-22] stops polling once revealed', async () => {
    jest.useFakeTimers();
    let pollCount = 0;
  ...
  it('[M1-22] resets the memorial when reused for a different, unresolved flight', async () => {
    ...
    const depsA = createFakeResolutionDeps({
      ...
      result: { outcome: 'died', body: null, place: 'Cape Cod fog bank', time: '3:04 AM' },
    ...
    await waitFor(() => expect(screen.getByText(/Cape Cod fog bank/)).toBeTruthy());
    ...
    expect(screen.getByTestId('flight-screen')).toBeTruthy();
    expect(screen.queryByText(/Cape Cod fog bank/)).toBeNull();
  });
});
